...
function filesFromFolder(
  dir: string,
  folder: string,
  fractionToKeep: number
): string[] {
  const f = fs.readdirSync(dir + folder);
  return f
    .slice(0, Math.round(f.length * fractionToKeep))
    .map((file) => dir + folder + "/" + file);
}
...
async function simplefaceData(task: Task): Promise<data.DataSplit> {
  ...
  return await new node.data.NodeImageLoader(task).loadAll(files, {
    labels: labels,
  });
}
...
async function cifar10Data(cifar10: Task): Promise<data.DataSplit> {
  ...
  const files = (await fs_promises.readdir(dir)).map((file) =>
    path.join(dir, file)
  );
  const labels = Range(0, 24)
    .map((label) => (label % 10).toString())
    .toArray();

  return await new node.data.NodeImageLoader(cifar10).loadAll(files, {
    labels: labels,
  });
}

class NodeTabularLoader extends data.TabularLoader<string> {
  loadTabularDatasetFrom(
    source: string,
...
async function soulBotData(soulbot: Task): Promise<data.DataSplit> {
  const dir = "../example_training_data/soulbot.csv";

  // TODO: can load data, so path is right.
  // console.log(await tf.data.csv('file://'.concat(dir)).toArray())
  const data = await new NodeTabularLoader(soulbot, ",").loadAll(
    ["file://".concat(dir)],
    {
      features: soulbot.trainingInformation?.inputColumns,
      labels: soulbot.trainingInformation?.outputColumns,
      shuffle: false,
    }
  );
...
export async function getTaskData(task: Task): Promise<data.DataSplit> {
  ...
  if (task.taskID === "soulbot") {
    return await soulBotData(task);
  }
